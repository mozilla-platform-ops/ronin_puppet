...
Describe "GPU Drivers" {
    It "Nvidia GPU driver installer exists in Windows\Temp" {
        (Get-ChildItem "C:\Windows\Temp\*grid*.exe" -ErrorAction SilentlyContinue) | Should -Not -BeNullOrEmpty
    }
}
...
Describe "VC++ Redistributables" {
    It "VC++ 2015-2022 x64 is installed" {
    ...
    It "VC++ 2015-2022 x86 is installed" {
        $vcc = Get-ItemProperty "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*" -ErrorAction SilentlyContinue |
            Where-Object { $_.DisplayName -like "Microsoft Visual C++ 2015-2022*x86*" }
        $vcc | Should -Not -BeNullOrEmpty
    }
}
