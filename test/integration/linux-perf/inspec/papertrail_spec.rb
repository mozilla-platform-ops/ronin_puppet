...
describe file('/etc/systemd/system/papertrail.service') do
  it { should exist }
  its('content') { should match /ExecStart=\/bin\/sh -c "journalctl -u check_gw -u run-puppet -u ssh  -f | ncat --ssl localhost 1111"/ }
end

describe file('/etc/systemd/system/papertrail-syslog.service') do
  it { should exist }
  its('content') { should match /ExecStart=\/bin\/sh -c "journalctl -t generic-worker -t run-start-worker -t sudo  -f | ncat --ssl localhost 1111"/ }
end
