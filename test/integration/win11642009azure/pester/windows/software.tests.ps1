...
Describe "Microsoft Tools Tester" {
    It "Windows 10 SDK is installed" {
        $sdk = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*" -ErrorAction SilentlyContinue |
            Where-Object { $_.DisplayName -like "*Windows Software Development Kit*" -or $_.DisplayName -like "*Windows SDK*" }
        $sdk | Should -Not -BeNullOrEmpty
    }
}
