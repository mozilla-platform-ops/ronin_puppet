...
ROLE_NAME = ENV.fetch('PUPPET_ROLE') { File.basename(File.expand_path('..', __dir__)) }.freeze
...
TASK_DRIVE = %w[
  win10-64-2009-alpha
  win10-64-2009-gpu-alpha
  win11-64-24h2-alpha
  win11-64-25h2-alpha
  win11-64-25h2-gpu-alpha
  win11-a64-25h2-builder-alpha
  win11-a64-25h2-tester-alpha
  win2022-64-2009-alpha
  win2025-64-24h2-alpha
].include?(ENV['WORKER_POOL_ID']) ? 'C:' : 'D:'
...
def expected_hiera_value(*keys)
  VARIANT_DATA.dig(*keys) || ROLE_HIERA.dig(*keys) || WINDOWS_DATA.dig(*keys)
end
