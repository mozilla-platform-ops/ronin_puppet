...
if ($single) {
	...
	ssh -o ConnectTimeout=5  -o StrictHostKeyChecking=no  $node_name "$local_script | Out-Null" 2> $Null
	#"powershell if (!(test-path $local_script)) {exit 97}; powershell -executionpolicy bypass -file $local_script" 2> $Null

	switch ($LASTEXITCODE) {
		0 { Write-Host "Successfully connecntion." }
		97 { Write-Host "Exit code 97. Local file is not present "}
...
if ($pool) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($pool_name)) {
        Write-Host "Single parameter is present. Node value is: $pool_name"
    }
	else {
