...
# sadly, we cannot run with `set -e`... variables defined by calls (including grep) fail
# because paths don't exist (and cause the script to exit) and `$? -eq 0` checks are done
#
# set -x
...
# check that we're root, exit if not
if [ "$(id -u)" != "0" ]; then
...
# variables and functions from the original script to include
# in the generated script
INCLUDE_FUNCTIONS = {"install_key", "handle_distro_upgrade", "update_bad_sources", "create_sources_lists",
                     "find_apt_trusted", "clean_sources_lists", "find_apt_sources", "uninstall_key"}
INCLUDE_VARIABLES = {"APT_GET", "APT_CONFIG", "REPOCONFIG", "REPOCONFIGREGEX"}

def extract_nodes(node, src, results):
    # Function definitions
    if node.get("Type") == "FuncDecl":
        func_name = node.get("Name", {}).get("Value")
        if func_name in INCLUDE_FUNCTIONS:
    ...
    # Assignments at the top level or inside commands
        ...
        for assign in node["Assigns"]:
            var_name = assign.get("Name", {}).get("Value")
            if var_name in INCLUDE_VARIABLES:
