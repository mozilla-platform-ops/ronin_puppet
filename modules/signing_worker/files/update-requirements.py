...
def run():
    ...
    with tempfile.TemporaryDirectory() as git_dir, tempfile.TemporaryDirectory() as req_dir:
        ...
        for name, config in scriptworker_config.items():
            ...
            for package_name in ("iscript", "scriptworker-client", "notarization_poller"):
                package_req = os.path.join(req_dir, f"{package_name}.txt")
                subprocess.run(["uv", "export", "--no-editable", "--no-annotate", "--no-dev", "--python", "3.8", "--package", package_name, "-o", package_req], cwd=git_dir)
