...
class win_taskcluster_cloud::generic_worker (
  ...
  exec { 'set_gw_app_dir':
    command => "${nssm_exe} AppDirectory ${generic_worker_dir}",
  }
  exec { 'set_gw_app_parameters':
    command => "${nssm_exe} AppParameters ${gw_app_parameters}",
  }
  exec { 'set_gw_display_name':
    command => "${nssm_exe} DisplayName \"Generic Worker\"",
  }
  exec { 'set_gw_description':
    command => "${nssm_exe} Description \"A taskcluster worker that runs on all mainstream platforms\"",
  }
  exec { 'set_gw_start':
    command => "${nssm_exe} Start ${gw_nssm_service_start}",
  }
  exec { 'set_gw_type':
    command => "${nssm_exe} Type ${gw_nssm_service_type}",
  }
  ...
  exec { 'set_gw_appexit':
    command => "${nssm_exe} AppExit ${gw_nssm_app_exit}",
  }
  exec { 'set_gw_restart_delay':
    command => "${nssm_exe} AppRestartDelay 0",
  }
  exec { 'set_gw_stdout':
    command => "${nssm_exe} AppStdout ${gw_log_file}",
  }
  exec { 'set_gw_stderror':
    command => "${nssm_exe} AppStderr ${gw_log_file}",
  }
