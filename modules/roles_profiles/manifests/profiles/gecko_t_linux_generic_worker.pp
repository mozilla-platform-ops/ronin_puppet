...
class roles_profiles::profiles::gecko_t_linux_generic_worker (
  String $worker_type,
  # These workers use one release for all Taskcluster binaries.
  # Add per-binary version parameters if a worker needs mixed releases.
  String $taskcluster_version,
