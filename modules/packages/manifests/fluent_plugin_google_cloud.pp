...
class packages::fluent_plugin_google_cloud {
    ...
    exec {
        'install plugin with agent ruby':
            ...
            command => "[ -e /usr/sbin/td-agent-gem ] && /usr/sbin/td-agent-gem install fluent-plugin-google-cloud --version ${version} || /opt/td-agent/usr/sbin/td-agent-gem install fluent-plugin-google-cloud --version ${version} || echo 'missing td-agent'",
