...
# System-level entries (SystemPolicyAllFiles) are handled by the
# org.mozilla.ci-tcc-pppc MDM configuration profile deployed via SimpleMDM.
# The system TCC database is SIP-protected on macOS 14/15 and cannot be
# written by root directly; sqlite3 writes to it silently fail or abort.
#
# Microphone + ScreenCapture for start-worker and generic-worker-multiuser
# are included here as user-DB fallbacks because both binaries are ad-hoc
# signed (no Developer ID) and can't be expressed in a PPPC profile with a
# stable CodeRequirement. The cdhash-anchored csreq is generated at runtime
# (see csreq_for_binary above) so the grant stays in sync when the worker
# binaries get version-bumped. The long-term fix is Developer ID signing —
# see https://github.com/taskcluster/taskcluster/issues/7413.
queries_14_user=(
